...
m2 <- m21 %>% rowwise() %>% mutate(hours=sum(q16_1, 
                                              q16_2, 
                                              q16_3,
                                              q16_4, 
                                              q16_5, 
...
m21 <- m21 %>% mutate(cucumber=car::recode(q20_2, "1:10='1-10';11:20='11-20';21:30='21-30';
                                           31:40='31-40';41:50='41-50';51:60='51-60';61:70='61-70';
...
m21_day1 <- m21 %>% filter(q16_1<6) %>% gather(key="t_day", value="p_day", 
...
m21 %>% filter(cucumber4!="NA") %>% 
  ct(cucumber4) %>% 
  ggplot(., aes(x=cucumber4, y=pct, fill= cucumber4)) + 
  ...
  geom_text(aes(label = round(pct, digits=2)*100), family="Jost", nudge_y = -.05) +
  labs(x="", y="", 
       title="Students Are Most Stressed About Academics\nand Least Stressed About Roommates",
...
m21 %>% filter(cucumber4!="NA") %>% 
  ...
  theme(text=element_text(family="Jost", size=12),
        ...
        axis.text.x = element_text(size=9),
        plot.title = element_text(hjust = .5)) +
  geom_text(aes(label = round(pct, digits=2)*100), family="Jost", nudge_y = -.02) +
  scale_y_continuous(labels = c("0.0"="0","0.1"="10%","0.2"="20%","0.3"="30%","0.4"="40%"))+
  labs(x="", y="", 
       title="Students Are Very Stressed",
       caption="Source: March 2021 Survey.\nNote: The Data Was Originally on a Scale of 1=100 but was Divided Into Four Groups; e.g. Cool as  a Cucumber is 1-25") +
   scale_fill_manual(values = wes_palette("GrandBudapest2"))



m21 %>% 
  group_by(yearf) %>% filter(q20_2!="NA") %>% filter(yearf!="NA") %>% 
  ggplot(., aes(x=q20_2, y=yearf, fill=yearf)) +
  geom_density_ridges(rel_min_height=.02, alpha=.8) +
  theme_minimal() +
  theme(text=element_text(family="Jost", size=12),
        legend.position = "blank",
        ...
        plot.title = element_text(hjust = .5)) +
  labs(y=NULL, x="", 
       title="Class of 2023 Is the Most Stressed",
       caption="Source: March 2021 Survey.") +
  scale_fill_manual(values = wes_palette("GrandBudapest2"))
...
m21 %>% 
  group_by(yearf) %>% filter(q20_2!="NA") %>% filter(yearf!="NA") %>% 
  ggplot(., aes(x=q20_2, y=yearf, fill=yearf)) +
  geom_density_ridges( alpha=.8, rel_min_height=.02) +
  theme_minimal() +
  theme(text=element_text(family="Jost", size=12),
        legend.position = "blank",
        panel.grid.major.y = element_blank(),
...
m21_str %>% 
  ...
  scale_y_continuous(limits = c(0,10), 
                     labels = c("Not at all Stressful", "2.5", "5", "7.5", "Very\nStressful")) +
  labs(x="", y="", 
       title="Students Are Most Stressed About Academics\nand Least Stressed About Roommates",
       caption="Source: March 2021 Survey.\nNote: Confidence Intervals Are Equivalent to a 90% Test of Statistical Significance When Two Are Compared") +
  gbh_fill()
...
m21_day1 %>% group_by(t_day,p_day) %>% 
  mean_ci(q20_2) %>% 
  ggplot(.,aes(x=p_day, y=mean, color=t_day, fill=t_day)) +
  geom_smooth()+
  theme_hc() +
  theme(text=element_text(family="Jost", size=12),
        ...
        axis.text.x = element_text(size=9),
        plot.title = element_text(hjust = .5)) +
  labs(x="Hours Spent Doing Each Activity Per Day",
       y= "Stress Levels",
       title = "There's No One Size Fits All Solution for Stress",
       caption = "Source: March 2021 Survey")+
  gbh_color() + gbh_fill()
...
```{r}
m21 %>% 
  group_by(drink) %>% filter(drink!="NA") %>%  
  mean_ci(q20_2, ci=.76) %>% 
  ggplot(., aes(x=drink , y=mean, fill= drink)) + 
  geom_col(alpha=.8, color="black", width = .75) + 
  geom_errorbar(aes(ymin=lower, ymax=upper), width=.1) +
  theme_hc() +
  theme(text=element_text(family="Jost", size=12),
        ...
        axis.text.x = element_text(size=9),
        plot.title = element_text(hjust = .5)) +
  scale_x_discrete(labels= c("1"="0","2"="1","3"="2","4"="3","5"="4","6"="5","7"="6","8"="7"))+
  geom_text(aes(label = round(mean, digits=2)), family="Jost", vjust=5) +
  labs(x="Days Binge Drinking per Week", y="Stress Level", 
       title="Binge Drinking More than Three Days a Week Causes a Spike in Stress",
       caption="Source: March 2021 Survey.\nNote: Confidence Intervals Are Equivalent to a 90% Test of Statistical Significance When Two Are Compared") +
  gbh_fill()
...
m21 %>% 
  group_by(q21_1f) %>% filter(q16_3!="NA") %>% 
  mean_ci(q16_3, ci=.76) %>% 
  ggplot(., aes(x=q21_1f , y=mean, fill= q21_1f)) + 
  geom_col(alpha=.8, color="black", width = .75) + 
  geom_errorbar(aes(ymin=lower, ymax=upper), width=.1) +
  theme_hc() +
  theme(text=element_text(family="O", size=12),
...
#Attending class
m21 %>% 
  group_by(attend) %>% filter(q16_2!="NA") %>% 
  mean_ci(q21_1, ci=.76) %>% 
...
#Social media
m21 %>% 
  group_by(sm) %>% filter(sm!="NA") %>%  
  mean_ci(q21_1, ci=.76) %>% 
  ggplot(., aes(x=sm , y=mean, fill= sm)) + 
  geom_col(alpha=.8, color="black", width = .75) + 
  geom_errorbar(aes(ymin=lower, ymax=upper), width=.1) +
  theme_hc() +
  theme(text=element_text(family="O", size=12),
  ...
  scale_y_continuous(limits = c(0,10), 
                     labels = c("Not At All\nStressful", "2.5", "5", "7.5", "Very\nStressful")) +
  labs(x="Number of Hours On Social Media", y=NULL, 
       title="Social Media",
       caption="Source: March 2021 Survey.\nNote: Confidence Intervals Are Equivalent to a 90% Test of Statistical Significance When Two Are Compared") +
  scale_fill_futurama()
